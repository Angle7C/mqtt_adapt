...
pub async fn create_client_with_connect(
    ...
    // 创建初始客户端实例
    let mut client = Client::new(socket, rx, router_event_sender);
    ...
    if n == 0 {
       return Err(anyhow::format_err!("Connection closed by client"));
    }
    ...
    match MqttPacket::read(&mut client.read_buf) {
        Ok(packet) => {
            if let MqttPacket::Connect(connect_packet) = packet {
                ...
                // 注册客户端到路由器
                router.register_client(client_id.clone(), tx.clone());
        ...
        Err(e) => {
            error!("Failed to parse CONNECT packet: {}", e);
            return Err(anyhow::format_err!("Failed to parse CONNECT packet: {}", e));
        }
    }
